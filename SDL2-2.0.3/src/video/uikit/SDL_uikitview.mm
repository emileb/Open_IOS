...
TouchControlsInterface *touchIF;
...
- (void)touchesBegan:(NSSet *)touches withEvent:(UIEvent *)event
{
    ...
    while (touch) {
        ...
        //touchIF->processPointer(P_DOWN, ((size_t)touch),locationInView.x, locationInView.y);
                ...
                for(i = 0; i < MAX_SIMULTANEOUS_TOUCHES; i++) {
                    if (finger[i] == NULL) {
                        finger[i] = touch;
                        if (touchIF)
                            touchIF->processPointer(P_DOWN, i,locationInView.x, locationInView.y);
                        break;
                    }
                }
...
- (void)touchesEnded:(NSSet *)touches withEvent:(UIEvent *)event
{
    ...
    while(touch) {
        ...
        for(i = 0; i < MAX_SIMULTANEOUS_TOUCHES; i++) {
            if (finger[i] == touch) {
                if (touchIF)
                    touchIF->processPointer(P_UP, i,locationInView.x, locationInView.y);
                finger[i] = NULL;
                break;
            }
        }
...
- (void)touchesMoved:(NSSet *)touches withEvent:(UIEvent *)event
{
    ...
    while (touch) {
        ...
        //touchIF->processPointer(P_MOVE, ((size_t)touch),locationInView.x, locationInView.y);
        
        int i;
        for(i = 0; i < MAX_SIMULTANEOUS_TOUCHES; i++) {
            if (finger[i] == touch) {
                if (touchIF)
                    touchIF->processPointer(P_MOVE, i,locationInView.x, locationInView.y);
                break;
            }
        }
...
SDL_bool UIKit_IsScreenKeyboardShown(_THIS, SDL_Window *window)
{
    ...
    return view.keyboardVisible?SDL_TRUE:SDL_FALSE;
}
